...
To evaluate individual variations in growth and fungicide sensitivity parameters, a simplified version of the bioenergetic model from @bart_2020 was developed.

We used an individual reaction norm approach [@dingemanse_2013] to compare growth rate across exposed and non-exposed individuals. As indicated by [@bart_2020], *A. caliginosa* growth is approximately linear with the structural volumetric length $L$ during the juvenile phase of the life-cycle. This structural volumetric length can be calculated as the cubic root of the wet weight of the organisms.
...
In parallel, the simulated pesticides effect can be expressed with a response ratio (Ln(RR), @eq-LnRR) and a coefficient of variation ratio (Ln(CVi), @eq-LnCVi).
...
```{r simdataplot, warning=F, message=F}
...
p_growth <- ggplot() +
  ...
  labs(
    ...
    subtitle  = "For the <span style = 'color:#5E81AC;'>non exposed</span> 
       and <span style = 'color:#BF616A;'>exposed</span> groups"
    )+
...
The priors cannot be defined according to the results of @bart_2020. Therefore we base the prior to our general knowledge about *A. caliginosa* :
...
-   We have no real *a priori* knowledge about the change inter-individual variation of growth. We therefore want a large prior for individual variations such as a normal distribution with a mean of 20% of the mean growth rate and a coefficient of variation of 100%. We are however sure of the existence of individual variation throught our expertise of the breeding culture and the data from @bart_2019.
...
priors <-
  ...
  prior(normal(2.32*0.2, 2.32*0.2), class = sd, nlpar = L0, lb = 0)+                           # <2>
  ...
  prior(normal(0.070*0.2,  0.070), class=sd,                                           # <5>
        coef=exposition0, group=ID, nlpar = a, lb = 0)+                                # <5>
  prior(normal(0.070*0.2,  0.070), class=sd,                                           # <6>
        coef=exposition1, group=ID, nlpar = a, lb = 0)+                                # <6>
...
```{r tableres}
#| tbl-cap: Different statistical values of the posterior distributions
...
```{r ctrlpredcheck, echo = T, warning=F}
#| fig-cap: Posterior predictive check. Fresh body weight of simulated non-exposed earthworms (blue points). Colored ribbons correspond to the different credibility intervals for the predicted growth for each individual. The thicker black lines represent the predicted body weight.
...
```{r trtpredcheck, warning=F, message=F}
...
```{r, include = F}
ggsave(
  filename="Modsim_compestimations.png", 
  plot=plot, 
  width=6, height = 4, 
  path=here::here("fig/")
  )
...
df_err <- data.frame(
  Parameter_name = c(as.list(df_res_VI_show[1])$variable, "LnRR", "LnCVi"),
...
df_err <- df_err |> 
  mutate(
    Simulated = signif(Simulated,2),
    Predicted = signif(Predicted,2),
    Per_error = round((Predicted-Simulated)/Simulated*100, 2)
  )
...
Parameter recovery is excellent at the population level, with absolute errors below 1.1%, and strong at the individual level, with absolute errors below 4%. Effect sizes -LnRR and LnCVi- are estimated with 12.5% and 14.7%, respectively.
