...
![*Aporrectodea caliginosa* life cycle. Figure taken from @bart_2018](../fig/Acali_lifecycle.png){#fig-AcaliLC}
...
df_growth_ctrl <- subset(df_growth_raw,exposition=="Ctrl")
df_growth_trt <- subset(df_growth_raw,exposition=="Trt")

df_growth_raw_mean <- df_growth_raw |> 
  aggregate(w~t+exposition, mean)

df_growth_ctrl_mean <- subset(df_growth_raw_mean,exposition=="Ctrl")
df_growth_trt_mean <- subset(df_growth_raw_mean,exposition=="Trt")
...
# Survival curves
df_growth_raw <- df_growth_raw |>          # <1>
  mutate(dead = case_when(                 # <1>
    status == "D" ~ 1,                     # <1>
    status %in% c("J", "S", "A") ~ 0)      # <1>
    )                                      # <1>

df_surv <- df_growth_raw |>                # <2>
  arrange(t) |>                            # <2>
...
p_surv <- ggplot(
  ...
  geom_step(linewidth = 1.2) +  # Utilise geom_step pour représenter les étapes de la courbe
  ...
  theme(
    ...
    panel.grid.major.y = element_line(color = "gray70", size = 0.3),  # Lignes principales plus visibles
    panel.grid.minor.y = element_line(color = "gray90", size = 0.2),  # Lignes secondaires plus subtiles
    )
...
plot_growth_ctrl <- ggplot(
  ...
  geom_line(color=col_blue, alpha=0.5)+
  geom_point(color=col_blue, alpha=0.5)+
  geom_line(
    data=subset(df_growth_mean, exposition=="Ctrl"),
    aes(
      x=t, 
      y=w, 
      group=NA
...
plot_growth_trt <- ggplot(
  ...
  geom_line(color=col_red, alpha=0.5)+
  geom_point(color=col_red, alpha=0.5)+
  geom_line(
    data=subset(df_growth_mean, exposition=="Trt"), 
    aes(
      x=t, 
      y=w, 
      group=NA
...
plot_growth <- ggplot()+
  ...
  geom_point_interactive(
    ...
    aes(
      x=t, y=w, 
      group=ID, 
      color=exposition, fill=exposition,
      tooltip = exposition, 
      data_id = exposition
        ), 
    alpha=0.5
...
plot_growth_tot <- plot_growth_ctrl + plot_growth_trt + plot_growth +
  plot_layout(guides="collect", ncol=2) +
  plot_annotation(title = "Growth monitoring of the <span style = 'color:#5E81AC;'>non exposed</span> and <span style = 'color:#f42404;'>exposed</span> groups") & 
  theme(
    legend.position = "none", 
    title=element_markdown(face="bold", size=10), 
    axis.title.x = element_text(face="plain")
    )
...
interactive_plot <- girafe(ggobj=plot_growth_tot) |> 
...
Our working dataset is shown @fig-growthdataset.

```{r, include=T, warning=F, message=F}
# ID of dead individuals before 14 days of experiment
dead <- subset(df_growth, t==14 & status == "D")$ID # <1>

df_growth_56 <- df_growth |> subset(t<57)
df_growth_56_alive <- df_growth_56 |> subset(!ID %in% dead) # <2>
```

1.  ID of dead individuals before 56 days of experiment
2.  Dataset of the first 56 days without those individuals
...
var.test(                                                   # <1>
  subset(df_growth_56_alive, exposition=="Ctrl" & t==56)$w, # <1>
  subset(df_growth_56_alive, exposition=="Trt"& t==56)$w    # <1>
  )                                                         # <1>

t.test(                                                 # <2>
  df_growth_56_alive$w ~ df_growth_56_alive$exposition, # <2>
  var.equal=F                                           # <2>
  )                                                     # <2>
```
